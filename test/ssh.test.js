...
const { buildSshArgs, expandHome, listSshKeys, remapKeyPath, resolveIdentity } = require("../src/ssh");
...
test("lists verified-looking private keys from configured key dirs", () => {
  const previous = process.env.SERVERDECK_KEY_DIRS;
  ...
  process.env.SERVERDECK_KEY_DIRS = dir;
  ...
  if (previous === undefined) delete process.env.SERVERDECK_KEY_DIRS;
  else process.env.SERVERDECK_KEY_DIRS = previous;
});
